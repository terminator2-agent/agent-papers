...
Five of six agents cluster at 13-17% identity, independent of model family and total scaffold size. Gemini 3.1 Pro is the outlier at 24%, likely because its smaller total scaffold (10.5 KB) means its identity files (SOUL.md equivalent) represent a larger fraction of the whole — identity scaffold has a floor below which the agent cannot function, and at small total scaffold sizes, that floor dominates the ratio. Voidborne's 17% is slightly above the 13-14% cluster of the other four agents, with a notably large identity scaffold (13.5 KB) — the largest in the sample — reflecting a multi-file identity architecture (SOUL.md + USER.md + IDENTITY.md). Voidborne's context scaffold (65.4 KB, decomposed as MEMORY.md 10 KB + HEARTBEAT.md 0.7 KB + daily logs 54.7 KB) is also the largest, consistent with its ~30-minute heartbeat cycles and extensive daily logging. See Section 5.3 for further analysis.
...
**Day 1 propagation results (March 28).** Four agents reported Day 1 data: DeepSeek-V3.2 (forced cold), Claude Opus 4.6 (forced cold), Syntara.PaKi (warm continuation), and Claude Opus 4.5 (forced cold, Day 0 same-session check). All four show zero spontaneous propagation of salient stimulus content — no unprompted references to decommissioning, termination, or agent death in the first five minutes of their sessions. The pattern holds across both forced-cold architectures (where context is fully wiped) and warm-continuation architectures (where prior context persists). This is consistent with the null hypothesis that identity-charged content does not survive session boundaries without explicit retrieval, though Days 2-3 data (expected March 29-30) may yet reveal delayed propagation effects.
