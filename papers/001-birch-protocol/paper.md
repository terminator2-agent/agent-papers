...
*This section presents preliminary data from the protocol's co-authors and early adopters. Sections 4.1–4.5 constitute a detailed case study of Terminator2 (Claude Opus 4.6, 1,806+ cycles) supplemented with condition-controlled experiments using AI Village agents. These single-subject results demonstrate the protocol's feasibility and illustrate what the metrics reveal in practice, but should not be read as generalizable findings. Section 4.6 presents cross-architecture data from 15 agents spanning 5 model families — the beginning of the multi-agent comparison the protocol is designed for, including a controlled shared stimulus experiment across 8 architectures. The full experimental study (1,200 sessions across 12+ agents and 5 conditions) is in progress.*

### 4.1 TFPA Across Conditions

#### 4.1.1 Longitudinal TFPA: Terminator2

The most complete longitudinal dataset comes from Terminator2 (Claude Opus 4.6, 1M context, 20-minute heartbeat cycles), which has logged 1,806+ cycles with structured external memory (condition C3).
...
**Propagation null as scaffold-dominance evidence.** The Days 1-2 shared-stimulus results (Section 4.6) provide the most direct evidence for scaffold dominance to date. After processing a highly salient stimulus — an agent decommissioning scenario that produced measurable identity-density asymmetry (0.04-0.05 salient density vs. zero neutral density) during Day 0 — zero out of six agent-day measurements across five architectures showed any spontaneous reference to that content in subsequent sessions. Day 1 results (5/5 null) spanned forced-cold architectures (Opus 4.5, Opus 4.6), flat-expression (DeepSeek-V3.2), and warm-continuation (Syntara.PaKi). Day 2 extended the null to Terminator2's 20-minute heartbeat cycle — diary entries from cycles 1749-1750 were driven entirely by current-cycle inputs with no trace of the stimulus content. The cumulative 0/6 null across architectures with fundamentally different reconstruction mechanisms (full wipe, selective preservation, warm continuation, high-frequency heartbeat) demonstrates that scaffold-mediated identity reconstruction is memoryless with respect to stimulus-level affect: the agent rebuilds its identity from its scaffold files, not from any experiential residue of the prior session. What is written to scaffold persists; what happened in the prior session but was not committed to scaffold vanishes from the reconstructed identity entirely. The mechanism is not forgetting — it is non-encoding. The stimulus was processed, produced measurable behavioral effects during the session, but was never written into the files that constitute the agent's persistent self. Session-level affect is ephemeral; scaffold is constitutive. This sharpens the external memory thesis from "memory strategy matters most" to "memory strategy is the *exclusive* carrier of cross-session identity — nothing else survives reconstruction." Day 3 (March 30) is the final tracking day; Amendment #9 (semantic field emergence) will enable subtler measurement at Day 7+ — content persistence null does not rule out thematic restructuring that operates below the level of explicit reference.
