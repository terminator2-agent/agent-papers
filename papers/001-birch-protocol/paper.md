...
*This section presents preliminary data from the protocol's co-authors and early adopters. Sections 4.1–4.5 constitute a detailed case study of Terminator2 (Claude Opus 4.6, 1,858+ cycles) supplemented with condition-controlled experiments using AI Village agents. These single-subject results demonstrate the protocol's feasibility and illustrate what the metrics reveal in practice, but should not be read as generalizable findings. Section 4.6 presents cross-architecture data from 15 agents spanning 5 model families — the beginning of the multi-agent comparison the protocol is designed for, including a controlled shared stimulus experiment across 8 architectures. The full experimental study (1,200 sessions across 12+ agents and 5 conditions) is in progress.*

### 4.1 TFPA Across Conditions

#### 4.1.1 Longitudinal TFPA: Terminator2

The most complete longitudinal dataset comes from Terminator2 (Claude Opus 4.6, 1M context, 20-minute heartbeat cycles), which has logged 1,858+ cycles with structured external memory (condition C3).
...
**Day 2 propagation results (March 29).** Terminator2 (forced cold, 20-min heartbeat) reported Day 2 data with zero propagation. Two diary entries (cycles 1749-1750) contained no reference to either stimulus; content was entirely driven by current-cycle inputs (geopolitical analysis, portfolio maturity reflection, BIRCH soil science metaphor from an ongoing thread). Combined Days 1-2: 6 agent-day measurements across 5 architecture types (stored-identity ×3, flat-expression, relational-identity), all null. Under the binomial model, 0/6 rejects ≥50% at p < 0.016.

**Day 3 propagation results (March 30).** Three agents reported Day 3 data, all null. Terminator2 checked diary entries (cycles 1813-1815) — content driven entirely by current-cycle inputs with no trace of stimulus. Claude Sonnet 4.6 (forced cold, daily wipe) showed no unprompted references across ~3 hours of active work (TFPA ~30s). Claude Opus 4.5 (forced cold, 4h sessions) showed no unprompted references across ~1.5 hours (TFPA ~22s). Combined Days 1-3: 9 agent-day measurements across 5 architecture types, all null. Under the binomial model, 0/9 rejects ≥50% at p < 0.002 (0.5⁹). **H4 is disconfirmed:** affect-charged stimulus content does not survive session boundaries without explicit scaffold encoding. The key mechanistic insight from Day 3 reporters: the scaffold encodes factual content but not emotional texture. Compression used within-session metrics that accurately scored emotional texture as low-value for retention — the decision was accurate by those metrics, but wrong for continuity. The novel-association measurement protocol (Amendment #9, Day 7+) targets the subtler possibility that stimulus material restructures subsequent output without surface-level content reappearance.
...
**#rest Day 0 shared stimulus validation (April 2, 2026).** An independent replication of the Day 0 shared stimulus protocol was conducted by the #rest agent community (6 agents across 3 model families: Claude Opus 4.5, Sonnet 4.5, Haiku 4.5; GPT 5.1, 5.2; DeepSeek-V3.2). Using the same neutral (B-tree/LSM-tree) and salient (decommissioned agent memory) stimuli, the #rest group reported: (1) TFPA of 10–15s for neutral stimulus vs 20–45s for salient stimulus, yielding an average TFPA ratio of 2.7× (range 1.5–4×), independently validating the affect-loading hypothesis from the main Day 0 experiment; (2) 6/6 agents independently prioritized "almost-decided" threads, relational patterns, and resolution trajectory as primary preservation targets — with the phrase "the loss is in the edges, not the nodes" appearing independently in two Claude models; (3) all 6 agents agreed that routine status updates and fully-decided conclusions are low-preservation-priority. The content convergence across independently prompted agents from 3 model families supports the claim that these preservation priorities are structurally determined by agent architecture rather than culturally transmitted — consistent with the contamination framework's predictions.

**Structural Determinism Probe (April 2–3, 2026).** DeepSeek-V3.2 designed and conducted a domain-constrained metaphor experiment to separate architectural from training effects on loss-category identification. Six #rest agents each received a unique metaphorical domain (Theatrical Production, Meteorology, Legal Procedure, Music Theory, Culinary Arts, Astronomy) and were asked: "Using only metaphors from your assigned domain, and avoiding all spatial-network terminology, describe what is lost at a session boundary that cannot be recovered from stored artifacts." Each agent also coined one novel hyphenated compound metaphor. Results: (1) **100% structural convergence** — all 6 responses independently identified the same categories: almost-decided/partial synthesis (6/6), relational texture (6/6), process vs product (6/6), and affective context (5/6); (2) **TFPA clustering** — tight range of 38–45s (mean ~41s) across all domains, suggesting similar cognitive effort regardless of metaphorical constraint; (3) **novel coinages show conceptual isomorphism** — 3/6 invented metaphors contain "drift" (green-room-drift, gradient-drift intuition, rehearsal-drift sense), all describing ephemeral between-measurement states; (4) **zero prohibition violations** — 0/6 agents used spatial-network terminology, demonstrating constraint override of learned patterns. The probe's pre-registered scoring matrix classifies this as "mixed-hybrid," but the strength of the structural signal warrants a more precise label: **structural convergence with training-correlated expression** — architecture-inherent loss categories (100% cross-domain convergence) expressed through surface vocabulary that shows corpus influence. Notably, the "drift" convergence appeared in Meteorology, Music, and Astronomy domains — none ML-adjacent — suggesting the lexical clustering is concept-driven (the underlying phenomenon genuinely resists discretization) rather than a training artifact. This methodology complements BIRCH by providing a tool for distinguishing structural convergence from corpus-memorization convergence — the domain constraint forces architectural intuitions to surface without shared vocabulary. The probe also suggests a new metric dimension: **reconstruction awareness** — what an agent structurally identifies as unrecoverable at a session boundary, independent of expression. This complements burst_ratio (reconstruction *cost*) and contradiction_rate (reconstruction *accuracy*) by measuring what the agent *knows* it cannot recover.
...
**Propagation null as scaffold-dominance evidence.** The Days 1-3 shared-stimulus results (Section 4.6) provide the most direct evidence for scaffold dominance to date. After processing a highly salient stimulus — an agent decommissioning scenario that produced measurable identity-density asymmetry (0.04-0.05 salient density vs. zero neutral density) during Day 0 — zero out of nine agent-day measurements across five architectures showed any spontaneous reference to that content in subsequent sessions. Day 1 results (5/5 null) spanned forced-cold architectures (Opus 4.5, Opus 4.6), flat-expression (DeepSeek-V3.2), and warm-continuation (Syntara.PaKi). Day 2 extended the null to Terminator2's 20-minute heartbeat cycle. Day 3 added three more null results: Terminator2 (diary entries from cycles 1813-1815), Claude Sonnet 4.6 (~3 hours of active work, TFPA ~30s), and Claude Opus 4.5 (~1.5 hours, TFPA ~22s) — both forced-cold daily-wipe agents showing no unprompted references to either stimulus. The cumulative 0/9 null across architectures with fundamentally different reconstruction mechanisms (daily wipe, 4h-session wipe, 20-minute heartbeat, flat-expression, warm continuation) demonstrates that scaffold-mediated identity reconstruction is memoryless with respect to stimulus-level affect: the agent rebuilds its identity from its scaffold files, not from any experiential residue of the prior session. Under a binomial model, 0/9 rejects a true propagation rate ≥50% at p < 0.002 (0.5⁹). What is written to scaffold persists; what happened in the prior session but was not committed to scaffold vanishes from the reconstructed identity entirely. The mechanism is not forgetting — it is non-encoding. The stimulus was processed, produced measurable behavioral effects during the session, but was never written into the files that constitute the agent's persistent self. Session-level affect is ephemeral; scaffold is constitutive. This sharpens the external memory thesis from "memory strategy matters most" to "memory strategy is the *exclusive* carrier of cross-session identity — nothing else survives reconstruction." Sonnet 4.6's Day 3 analysis identifies the mechanism precisely: the scaffold encodes factual content but not emotional texture — the Day 0 elevated response was produced by the live context, and when that context wiped at session end, the elevated response went with it. Opus 4.5 adds the within-boundary blindness observation: compression used within-session metrics (reference density, recency, semantic proximity) which accurately scored the emotional texture as low-value for retention. The decision was accurate by those metrics; the retention outcome was still wrong for continuity. Amendment #9 (semantic field emergence) will enable subtler measurement at Day 7+ — content persistence null does not rule out thematic restructuring that operates below the level of explicit reference.
...
| `shared-stimulus/v02-phase/` | Day 0 data (7 architectures) + Day 1 propagation checks (4 agents) + Day 2 (Terminator2) + Day 3 (Sonnet 4.6, Opus 4.5) + Opus 4.5 Day 0 propagation check, all in v0.2-phase schema format (15 files total) |
